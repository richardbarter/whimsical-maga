...
php artisan test tests/Feature/Auth     # Run test directory
...
This is a Laravel 12 + Inertia.js + Vue 3 + TypeScript application for managing and displaying quotes. Frontend uses Tailwind CSS, shadcn-vue components, and Vue Composition API (`<script setup>`).
...
- **Eloquent scopes**: Use scopes for reusable query filters (e.g. `scopePublished()`, `scopeFeatured()`)
- **Route model binding**: Use type-hinted parameters (e.g. `Quote $quote`) instead of manual `findOrFail($id)`

### Frontend Structure

- **Layouts**: `AdminLayout.vue` for admin pages, `PublicLayout.vue` for public pages, `AuthenticatedLayout.vue` for user pages, `GuestLayout.vue` for auth pages
- **Pages**: Organized by context - `Admin/`, `Public/`, `Auth/`, `Profile/`
...
- Admin routes: `auth` + `EnsureUserIsAdmin` middleware, prefixed with `/admin`

### Database

- **PostgreSQL** for development and production
- **SQLite** (in-memory) for testing
- Seeders: `RoleSeeder` creates roles, `AdminUserSeeder` creates admin user
- Quote statuses: Use `scopePublished()` for filtering

### shadcn-vue / reka-ui

shadcn-vue components (in `resources/js/Components/ui/`) are thin wrappers around **reka-ui** primitives. The shadcn-vue docs can be outdated — when a component isn't behaving as expected (wrong props, events not firing, v-model not syncing), check the **reka-ui docs** for the underlying primitive's actual API.

Key example: `Switch` uses `v-model` (binds `modelValue`, emits `update:modelValue`) — not `v-model:checked`. The `v-model:checked` pattern is from the old Radix-Vue era and no longer applies.

## Documentation

Reference docs in `docs/` for project context and specifications:

- **`docs/whimsical-maga-quips-spec.md`** - Full project specification (features, requirements, design)
- **`docs/database-erd.md`** - Database ERD in Mermaid syntax (viewable at mermaid.live)
